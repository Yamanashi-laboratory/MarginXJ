...
`.msi` はスタートメニューに `MarginXJ` を登録し、デスクトップのショートカットは**インストール時に
作るかどうかを尋ねます**。ショートカットが作られるのはウィンドウ版だけです（CLI 版は端末から
打つもので、ダブルクリックしても引数が無いというメッセージが一瞬出て終わるためです）。
`.deb` も同様にメニュー項目を登録します。
...
**サイクル数の上限は変更できます**（GUI では CGM 選択時に現れる入力欄、CLI では `--cycles`）。
既定の 500 は元コードの値です。ただし**短くしても下限があります。** 最適化は前後に必ず素子全体の
測り直しを数回行い、`MUX_clked` では 1 回あたり約 34 秒かかるためです。実測は次のとおりです。

| 指定 | 所要時間 | 内訳 |
|---|---|---|
| `--cycles 5` | 3 分 48 秒 | 測り直し 3 回 + 500 シミュレーション |
| `--cycles 20` | 6 分 55 秒 | 測り直し 4 回 + 2,000 シミュレーション |
| 既定（500、105 サイクルで停止） | 3〜16 分 | 乱数依存で毎回変わります |

**試行回数（1 サイクルあたり 100 回）は変更できません。** 歩留まりの判定が「生き残った試行の個数」と
閾値 60 の比較で行われており、これが 60 % を意味するのは試行が 100 回だからです。減らすと閾値が
到達不能になり、spread が一度も広がらず最適化が働かなくなります。
